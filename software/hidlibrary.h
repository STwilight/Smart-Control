const char idstring[] = "VID_03EB&PID_204F";
...
template<typename T> string HIDLibrary<T>::GetDevicePath(HDEVINFO hInfoSet, PSP_DEVICE_INTERFACE_DATA oInterface)
{
		...
		if (!SetupDiGetDeviceInterfaceDetail(hInfoSet, oInterface, 0, 0, &nRequiredSize, 0))
		{
			...
			{
				// Преобразование wchar_t > char > string
				char char_s[255] = "";
				wchar_t *wchar_s = oDetail->DevicePath;
				wcstombs(char_s, wchar_s, 255);
...
template<typename T> int HIDLibrary<T>::EnumerateHIDDevices()
{
        ...
        while (SetupDiEnumDeviceInterfaces(hInfoSet, 0, &m_hidguid, nIndex, &spd))
		{
			...
			// Преобразование char > wchar_t
			wchar_t *wpath;
			mbstowcs(wpath, path.c_str(), path.length());

			if(path.find(idstring) != string::npos)
			{
				HANDLE h = CreateFile(wpath,GENERIC_READ,0,0,OPEN_EXISTING,FILE_FLAG_OVERLAPPED,0);
...
template<typename T> int HIDLibrary<T>::SendData(T* data)
{
   ...
   int len=datasize+1;
	...
	// Преобразование char > wchar_t
	wchar_t *wdev;
	mbstowcs(wdev, m_ConnectedDevice.c_str(), m_ConnectedDevice.length());

   HANDLE h = CreateFile(wdev,GENERIC_READ,0,0,OPEN_EXISTING,FILE_FLAG_OVERLAPPED,0);
   if(h != INVALID_HANDLE_VALUE)
   {
...
template<typename T> int HIDLibrary<T>::DeviceConnected()
{
   HANDLE h = CreateFile(m_ConnectedDevice.c_str(),GENERIC_READ,0,0,OPEN_EXISTING,FILE_FLAG_OVERLAPPED,0);
   if(h != INVALID_HANDLE_VALUE)
   {
